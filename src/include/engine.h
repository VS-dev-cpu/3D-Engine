...
public:
    ...
    void add(char *path);
    void add(lightType type, color3f c, float strength, vec3d position, vec3d direction);

    void tri(triangle tri);
};
...
void engine::add(lightType type, color3f c, float strength, vec3d position, vec3d direction)
{
    light l;
    l.type = type;
    l.color = c;
...
void engine::tri(triangle tri)
{
    objs[0].meshData.tris.push_back(tri);
}